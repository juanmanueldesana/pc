...
class ContactFile_AccountsExpressions {
  ...
  public static class IteratorEntryExpressionsImpl extends ScreenExpressionsImpl {
    ...
    function initialValue_7 () : java.lang.String {
      return item.AccountContacts.where(\ a -> a.Contact == contact)*.Roles.sortBy(\ acr -> acr.DisplayName).join(", ")
    }
...
var roles : java.lang.String = (item.AccountContacts.where(\ a -> a.Contact == contact)*.Roles.sortBy(\ acr -> acr.DisplayName).join(", "))
return accountHolderContact.DisplayName
    }
