...
<entity
  ...
  entity="${exclusion.EntityName}"
  type="effdated"
  desc="${exclusion.Description}"
  effDatedBranchType="PolicyPeriod"
  exportable="true"
  final="false"
  platform="false"
  loadable="false"
  table="${exclusion.TableName}">
  <implementsEntity
    name="Exclusion"/>
  <implementsInterface
    iface="gw.api.domain.ExclusionAdapter"
    impl="${exclusion.LOBPackageName}.${exclusion.ExclusionAdapterImplName}"/>
  <implementsInterface
    iface="gw.api.logicalmatch.EffDatedLogicalMatcher"
    impl="${exclusion.LOBPackageName}.${exclusion.ExclusionMatcherImplName}"/>
${EntityColumn.renderToString(exclusion.FkFieldToCoverable)}
  <column
    name="BooleanTerm1"
    type="bit"
