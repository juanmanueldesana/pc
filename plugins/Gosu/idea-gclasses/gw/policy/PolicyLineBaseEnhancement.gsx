...
enhancement PolicyLineBaseEnhancement : PolicyLine {
  ...
  function denormalizeReferenceDates() {
    ...
    coverables.each(\elt -> {
      elt.denormalizeCoverableReferenceDate()
      elt.VersionListUntyped.AllVersionsUntyped
          .where(\untyped -> untyped.EffectiveDate > elt.EffectiveDate and untyped.EffectiveDate < periodEnd)
          .each(\future -> {(future as Coverable).ReferenceDateInternal = elt.ReferenceDateInternal})
      } )
